    ...
    return (
        ...
        <>
            ...
            {offerData.offer.map((item) => {
                if (item.category === "taniecrekreacyjny" && window.innerWidth < 825) {
                    return <Offers id={getIDHandler(item.title)} title={item.title} subtitle={item.subtitle} desc={item.desc} image={item.url} />
                }
                if (item.category === "taniecrekreacyjny" && offerData.offer.indexOf(item) % 2 === 0 && window.innerWidth >= 825) {
                    return <Offers id={getIDHandler(item.title)} title={item.title} subtitle={item.subtitle} desc={item.desc} image={item.url} />
                }
                if (item.category === "taniecrekreacyjny" && offerData.offer.indexOf(item) % 2 === 1 && window.innerWidth >= 825) {
